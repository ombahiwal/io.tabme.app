...
class Menu6 extends Component {
    ...
    componentDidMount(){
        ...
        var subcategories;
        var subcats;
        this.setState({menu_rendered: <div className="menu-container background-white margin-btm">
        <Waypoint
             topOffset = "120px"
             bottomOffset ="80%"
     ...
     {this.state.menu.categories.map((cat)=>{
         ...
         return(<div>
             ...
             topOffset = "140px"
             bottomOffset ="80%"
             onEnter={()=>{ this.setState({active_category:cat})}}
            /> 
         <div className="col-12 background-light" style={{padding:'0px'}}>               
         <Grid>
         ...
         {subcats.map(((subcat)=>{
             return(
                 <div key={subcat}>
                 <p className="menu-subcategory"><center><b>{subcat}</b></center></p>
                 <Grid>
                        {this.state.menu.dishes.map((dish)=>{
        ...
        <Waypoint
             bottomOffset = "82%"
             topOffset ="110px"
             onLeave={()=>{ this.setState({active_category:cat})}}
         />
         </div>
         </div>);
  ...
  {/* </Accordion> */}
                ...
                <div className="col-12 m-2"> 
                    ...
                    <div className="col-12"><h4><b>{t('your_tab')}</b></h4></div>
                        ...
                        <Grid>
                            ...
                            <div>
                            <Link to="/cart"> 
                                <Button size="small" variant="outline-dark">{t('proceed_btn', {text:""})} &nbsp;<small><b><CurrencySymbol/>{this.renderCartTotal()}</b></small></Button>
                            </Link>  
                            </div>
                            <div>
                                <hr/>
    ...
    renderCustomisationModal(){
        ...
        return(<Modal id="modal-2" isOpen={this.state.isOpenCustom} onRequestClose={()=>{this.ModalCustomOnClose();  this.current_dish_optionset=[]; 
               ...
               <b><CurrencySymbol/> {dish.price.toFixed(2)}</b> <br/>
                ...
                <div className="col-12">
                                                ...
                                                {option.values.map((opt, idx)=>{
                                                    return(
                                                        <div key={opt.value}className="row">
                                                            ...
                                                            <div className="col-6">
                                                               <input type="radio" className="form-check-input" onChange={()=>{this.onClickDishOption(optn, opt._id)}} value={opt._id} name={option.title} id={option.title}/> 
                                                    ...
                                                    {option.values.map((opt, idx)=>{
                                                        ...
                                                        return(
                                                            <div key={opt.value} className="row">
                                                                ...
                                                                <div className="col-5"><small>+ <CurrencySymbol/> 
                        ...
                        {requiremsg && <br/>}
                    {requiremsg && <small><b>*</b> {this.checkDishAddButtonState() && <span style={{color:'red'}}>{t('required')}</span>} {!this.checkDishAddButtonState() && t('required')}</small>}
                        {showBar && <hr/>}
                    <div className="row">
                        <div className="col-4 no-padding-float-left" >
                            <ButtonGroup size="sm">
                                <Button variant="outline-secondary" onClick={()=>{this.onChangeCurrentDishCount(false)}} ><FaMinus/></Button>
                                <Button variant="outline-secondary"><b>{this.current_dish_count}</b></Button>
                                <Button onClick={()=>{this.onChangeCurrentDishCount(true)}}  variant="outline-secondary" label="+"><FaPlus/></Button>
                            </ButtonGroup>
                        </div>
                        <div className="col-8 no-padding-float-right" > 

                    
                                <Button  disabled={this.checkDishAddButtonState()}  variant="dark" className="wide-btn" onClick={()=>{this.addDishtoCart(dish, this.current_dish_count);}}>
    ...
    renderTotal(){
            return(
                  ...
                  <div className="col-12">
                  <div className="col-3 col-lg-2 col-md-2 no-padding-float-left">
                        {/* <h3><FaShoppingBasket/><sup><Badge pill variant="outline-brand">{this.state.itemTotal}</Badge></sup></h3>  */}
                        <span>{this.renderCartDrawer()}</span>
                        
                    </div>
                    
                  <div className="col-9 col-lg-10 col-md-10 no-padding-float-right" >
                    
                    <Link to="/cart"> 
                        <Button style={{width:'100%'}} className="wide-btn" variant="dark"><b>{t('proceed_btn', {text:""})}</b>&nbsp;<small><b><CurrencySymbol/>{this.renderCartTotal()}</b></small></Button>
                    </Link>
                </div>
    ...
    renderAllergenModal(){
       ...
       <small><b>{this.dishVegDisplay(this.state.dishInfo.veg)}</b></small>
        <p>{this.dishCatBreadCrumb(this.state.dishInfo.category, this.state.dishInfo.subcategory)}</p>
        <p><small>{this.state.dishInfo.description}</small></p>
    ...
    render(){
        return ( 
            ...
            <div className="col-12 no-padding-float-left" >
                ...
                <MenuCategories activecat={this.state.active_category} categories={this.state.menu.categories}/>
