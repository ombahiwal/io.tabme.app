       ...
       return( <small>
                <InputGroup  size="sm" className="mb-3">
                    <FormattedMessage id='cart_enter_promo' defaultMessage="Enter Code...">
                            {(placeholder)=><FormControl disabled={promo.valid} aria-label="Small" aria-describedby="inputGroup-sizing-sm" value={promo.value} placeholder={placeholder} onChange={onChangePromo}/>}
                        </FormattedMessage>
                        <InputGroup.Append>
                            <Button disabled={promo.valid} style={alert.style} variant="outline-secondary" onClick={handlePromoSubmit}><small><b>{t(alert.response)} {promo.valid  && <FaRegTimesCircle onClick={()=>{removeCode()}} />}</b></small></Button>
                        </InputGroup.Append>    
                </InputGroup>
